...
// 캐릭터를 링크로 넘길 때는 값 대신 목록 번호로만 주고받아, 목록에 없는 값은 들어올 수 없게 합니다.
const AVATAR_CODE = [
  ["skin", AVATAR.skins],
  ["hairColor", AVATAR.hairColors],
  ["blush", AVATAR.blushes],
  ["hair", AVATAR.hairByGender.neutral],
  ["expression", AVATAR.expressions],
  ["accessory", [null, "sunglasses", "strawhat", "cap", "flower", "ribbon"]],
];

export const avatarToCode = (avatar) => AVATAR_CODE.map(([k, list]) => list.indexOf(avatar[k]));

export function avatarFromCode(code) {
  if (!Array.isArray(code) || code.length !== AVATAR_CODE.length) return null;
  const avatar = {};
  for (let i = 0; i < AVATAR_CODE.length; i++) {
    const [k, list] = AVATAR_CODE[i];
    if (!Number.isInteger(code[i]) || code[i] < 0 || code[i] >= list.length) return null;
    avatar[k] = list[code[i]];
  }
  return avatar;
}
