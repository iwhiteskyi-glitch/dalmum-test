 ...
 * 위쪽은 이름 카드(+동행자), 아래쪽은 인사말·명소·음식 — 사진 한 장만 저장해도 여행 중에
 * 꺼내 볼 수 있게. 동행자가 많아 내용이 넘치면 세로로만 조금 길어집니다.
...
function paint(ctx, data, H, draw) {
  const { members, country, city, phrases, urlText, displayFont } = data;
  const [main, ...mates] = members;
  const { card } = main;
  const group = mates.length > 0;
  const gaeguFamily = `${displayFont}, ${SANS}`;
  ...
  // 캐릭터 + 닉네임
  const avatarSize = group ? 160 : 220;
  if (draw) drawAvatar(ctx, card.avatar, (W - avatarSize) / 2, y, avatarSize, C.tealSoft);
  y += avatarSize + 16;
  fitFont(ctx, `${main.nick} 님의 여행 이름`, CW, 27, 20, 700);
  ctx.fillStyle = C.muted;
  if (draw) drawCenter(ctx, `${main.nick} 님의 여행 이름`, y);
  y += 40;

  // 이름
  const nameSize = fitFont(ctx, card.pronunciation_kr, CW, 116, 64, 700, gaeguFamily);
  ctx.fillStyle = C.ink;
  ...
  ctx.fillStyle = C.muted;
  if (draw) drawCenter(ctx, nameLocalLine(card), y);
  y += 48;
  ...
  ctx.fillStyle = C.ink;
  for (const l of wrapLines(ctx, card.blurb, CW, group ? 1 : 2)) {
    if (draw) drawCenter(ctx, l, y);
    y += 48;
  }
  y += 14;
  ...
  y = chipRows(ctx, chips, y, 54, 12, draw);
  ...
  if (group) {
    ...
    ctx.font = sans(27, 800);
    ctx.fillStyle = C.ink;
    if (draw) drawCenter(ctx, "함께 떠나는 여행 메이트", y);
    y += 50;
    const colW = Math.min(220, CW / mates.length);
    const startX = (W - colW * mates.length) / 2;
    const mateAvatar = 104;
    mates.forEach((m, i) => {
      const cx = startX + colW * i + colW / 2;
      if (!draw) return;
      drawAvatar(ctx, m.card.avatar, cx - mateAvatar / 2, y, mateAvatar, C.tealSoft);
      fitFont(ctx, m.card.pronunciation_kr, colW - 16, 44, 26, 700, gaeguFamily);
      ctx.fillStyle = C.ink;
      drawCenter(ctx, m.card.pronunciation_kr, y + mateAvatar + 10, cx);
      fitFont(ctx, m.nick, colW - 16, 23, 16, 700);
      ctx.fillStyle = C.muted;
      drawCenter(ctx, m.nick, y + mateAvatar + 62, cx);
    });
    y += mateAvatar + 62 + 30;
  }
  y += group ? 20 : 36;
  ...
  for (const p of phrases) {
    if (draw) {
      ...
      fitFont(ctx, p.text_local, maxW, 29, 20, 800, localFamily);
      ctx.fillStyle = C.ink;
...
export async function buildTravelCard(data) {
  if (document.fonts?.load) {
    try {
      ...
      await document.fonts.load(`700 116px ${data.displayFont}`, `${names}여행 치트시트`);
    } catch {
