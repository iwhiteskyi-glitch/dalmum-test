...
// 카드에 직접 그려 넣는 고정 문구. result.parts[].name/meDesc/targetDesc/comment는
// analyzePair()가 이미 locale에 맞게 만들어주므로 여기서 건드릴 필요가 없고,
// 이 카드 자체가 그리는 제목/라벨류만 언어별로 나눕니다.
const STRINGS = {
  ko: {
    brand: SITE.name,
    headline1: "우리 이 정도로",
    ...
    me: "나",
    them: "대상",
    ctaLine: "너랑 나도 비교해 볼까?",
    footerTag: "재미로 보는 닮음 비교",
  },
  en: {
    brand: "Dalmum",
    headline1: "We're THIS",
    headline2: "alike?!",
    overallLabel: "Overall similarity",
    bestPrefix: "Most alike:",
    bestBadge: "Best match",
    me: "Me",
    them: "Them",
    ctaLine: "Want to see how alike you are?",
    footerTag: "a fun look-alike comparison",
  },
};

export async function buildShareCard(result, meSrc, targetSrc, variant = "summary", locale = "ko") {
  const detailed = variant === "detailed";
  const s = STRINGS[locale] || STRINGS.ko;
  ...
  ctx.font = font(34);
  ctx.fillText(s.brand, 540, 148);
  ctx.fillStyle = INK;
  ctx.font = font(52);
  ctx.fillText(s.headline1, 540, 230);
  ctx.fillText(s.headline2, 540, 296);

  // 아바타 + ×
  const [meImg, tgImg] = await Promise.all([loadImage(meSrc), loadImage(targetSrc)]);
  drawAvatar(ctx, meImg, 325, 440, 135);
  drawAvatar(ctx, tgImg, 755, 440, 135);
  ctx.fillStyle = INK;
  ctx.font = font(52);
  ...
  ctx.fillText(s.overallLabel, 540, 806);

  // 가장 닮은 부위 (흰색 알약)
  const bestPart = result.parts.find((p) => p.isBest) || result.parts[0];
  ctx.font = font(32, 700);
  const bestText = `${s.bestPrefix} ${bestPart.name} · ${bestPart.score}%`;
  ...
  if (detailed) {
    ...
    detailBlocks.forEach((block, i) => {
      ...
      drawCol(col1X, meClauses, img.me, s.me, ACCENT);
      drawCol(col2X, targetClauses, img.target, s.them, FOCUS);
    });
  } else {
  ...
  ctx.fillStyle = ACCENT;
  ctx.font = font(30, 800);
  ctx.fillText(s.ctaLine, 540, bottomY + 46);
  ctx.fillStyle = MUTED;
  ctx.font = font(24, 600);
  const domain = SITE.url.replace(/^https?:\/\//, "");
  ctx.fillText(`${domain} · ${s.footerTag}`, 540, bottomY + 90);
