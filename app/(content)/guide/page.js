  ...
  return (
    <article className={styles.article}>
      ...
      <div className={styles.prose}>
        <h2>1. 사진 두 장 올리기 — 얼굴은 자동으로 맞춰져요</h2>
        <p>
          첫 화면에서 <strong>내 사진</strong>과 <strong>비교 대상 사진</strong>을 각각
          올립니다. 네모 칸을 눌러 파일을 선택하거나, 사진 파일을 칸 위로 끌어다
          놓으면(드래그 앤 드롭) 됩니다. 사진을 올리면 <strong>얼굴을 자동으로 찾아
          점선 동그라미 안에 맞춰줍니다</strong> — 대부분은 이 상태로 바로 진행해도
          됩니다.
        </p>
        <p>
          사진에 <strong>여러 사람이 함께 나온 경우</strong>에는 감지된 얼굴들이 작은
          동그란 썸네일로 나열됩니다. 그중 비교하고 싶은 얼굴을 탭(클릭)하면 그 사람
          기준으로 다시 맞춰줍니다.
        </p>

        <h2>2. (필요할 때만) 위치 직접 조정하기</h2>
        <p>
          자동으로 맞춰진 위치가 마음에 안 들면 직접 조정할 수 있습니다. 슬라이더로
          확대·축소하고, 사진을 <strong>PC에서는 마우스로 바로 드래그</strong>,{" "}
          <strong>휴대폰에서는 두 손가락으로 밀어서</strong> 이동합니다(한 손가락은
          화면 스크롤 그대로 사용). 사진을 잘못 올렸다면 사진 오른쪽 위{" "}
          <strong>✕ 버튼</strong>으로 지우고 다시 올리면 됩니다.
        </p>
        <p>
          두 사진 모두 올리고 나면 <strong>분석 시작</strong> 버튼이 활성화됩니다.
        </p>

        <h2>3. 결과 확인하기</h2>
        <p>
          눈·눈썹·코·입·얼굴형·이목구비 배치 6가지를 순서대로 확인하는 짧은 로딩 화면
          다음, <strong>전체 닮음도 %</strong>와 항목별 점수, 그리고 각 부위를 잘라
          나란히 놓은 비교 이미지가 나옵니다. 가장 많이 닮은 부위에는 🏆 표시가 붙습니다.
        </p>
        ...
        <p>
          ...
          다시 해보고 싶으면 <strong>다시하기</strong> 버튼으로 처음부터 시작할 수
          있습니다.
        </p>
